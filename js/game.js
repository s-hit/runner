...
// var danmakuContainer = document.getElementById("danmaku");
...
var danmakuContent = [
    ...
    "相亲相爱一家人",
    "他真的好温柔，妈妈真的哭死",
    "男妈妈怎么了？男妈妈怎么了？男妈妈怎么了？",
    "彼此双向奔赴✨",
    "sukisukidaisuki",
    "温柔阳光治愈暖系大男孩，我🉑了",
];
var danmakuCount = danmakuContent.length;

var danmakuGameOver = [
    "季宝，我的季宝😭",
    "季宝我的季宝😭季宝我的季宝😭季宝我的季宝😭",
    "小幸，我的小幸😭",
    "儒儒子，我的儒儒子😭",
    "小幸，你快回来，妈妈想你😭",
    "家乡的撒库拉，再也看不到了😭",
    "季さん、家乡の桜花开了、哥哥姐姐👫都在家等你😭",
    "家乡的鲤鱼旗🎏随风飘扬，风铃🎐挂在窗檐下，风一吹，叮叮当当听",
    "他连撞死都那么温柔究竟经历了什么😭",
    "随喜赞叹🙏",
    "今天放生了一个小幸",
    "不要停下来啊！😭",
...
var danmakuGameOverCount = danmakuGameOver.length;
...
startListening: function() {
    ...
    // Touch / pointer.
    // danmakuContainer.addEventListener(Runner.events.TOUCHSTART, this);
...
onKeyDown: function(e) {
    ...
    if(e.target != this.detailsButton) {
        ...
        if(this.crashed && e.type == Runner.events.TOUCHSTART &&
            (e.currentTarget == this.containerEl || e.currentTarget == this.touchController)) {
            this.restart();
        }
    }
...
gameOver: function() {
    ...
    for (let i = 0; i < cnt; i ++) {
        setTimeout(function () {
            danmaku.emit({
                text: danmakuGameOver[Math.floor(Math.random() * danmakuGameOverCount)],
                style: {
                    fontSize: '20px',
                    color: '#ffffff',
    ...
    function updateRanking(user, distanceRan) {
        AV.Leaderboard.updateStatistics(user, {
            'score' : distanceRan ? Math.round(distanceRan * 0.025) : 0,
            'score_sum' : distanceRan ? Math.round(distanceRan * 0.025) : 0
        }).then(() => {
            AV.Leaderboard.createWithoutData('score').getResultsAroundUser(user, {
                limit: 5,
                ...
                rankings.forEach(ranking => {
                    danmaku.emit({
                        mode: 'bottom',
                        text: `${String(ranking.rank + 1)} - ${ranking.user.attributes.username} (${ranking.value})`,
                        style: ranking.user.attributes.username == user.get('username') ? 
                        {
                            fontSize: '20px',
                            color: '#ff0000',
    ...
    AV.User.logIn(id, id).then((user) => {
        updateRanking(user, distanceRan);
    }, () => {
...
startJump: function(speed) {
    ...
    danmaku.emit({
        text: danmakuContent[Math.floor(Math.random() * danmakuCount)],
        style: {
            fontSize: '20px',
            color: '#ffffff',
            textShadow: '-1px -1px #000, -1px 1px #000, 1px -1px #000, 1px 1px #000'
